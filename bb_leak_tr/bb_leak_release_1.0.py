...
def _(mo):
    mo.md(
        ...
        My goal is to show you how to analyze this kind of data with practical code that you can understand, adapt, and reapply. In this specific notebook, I didn’t use Jupyter; I used Marimo, and I recommend checking out the [Marimo documentation](https://marimo.io/docs) to understand how to run this notebook locally.




        **Disclaimer: This notebook is my personal work to demonstrate how Pythonand GenAI can be used for analyzing chat logs. It is not affiliated with or supported by my employer.**
        ...
        *`<shameless_plug>` If you want to take your skills even further, check out my training on GenAI for Threat Intel: [https://store.securitybreak.io/ctiai](https://store.securitybreak.io/ctiai) `</shameless_plug>`*
...
def _():
    ...
    return Counter, alt, datetime, json, np, pd, re
...
def _(alt, df, mo):
    ...
    chart1
    return chart1, format_chart, hourly_chart
...
def _(alt, df):
    ...
    return day_order, heatmap, heatmap_data
...
def _(alt, df, mo):
    ...
    return chart, monthly_counts


@app.cell
def _(alt, df, format_chart, mo):
...
def _(alt, df, format_chart, mo):
    ...
    chart3
    return chart3, top_users, top_users_chart
...
def _(mo):
    mo.md(
        ...
        Last but not least, my prompt is short and direct because I am using  the reasoning model o1, which does not require a complex prompt to provide accurate results. The context itself will be long enough.
        """
    )
    return
...
@app.cell
def _(RunnablePassthrough, StrOutputParser, hybrid_retriever, llm, prompt):
    # format documents
    ...
    return (
        chat_with_documents2,
        format_docs2,
        rag_chain2,
        rag_chat_model2,
        retrieve_and_format2,
    )


@app.cell
def _(mo, rag_chat_model2):
...
@app.cell
def _(mo):
    mo.md("""And boom we now have a fully customisable rag grounded for our specific data with the original data to verify.""")
    return
...
@app.cell
def _(hybrid_retriever, rag_chain2):
